...
const StructuredView: React.FC<StructuredViewProps> = ({ data }) => {
  ...
  const medicines = data.medical_info.medicines ?? [];
  const diagnostics = data.medical_info.diagnostics_tests ?? [];
  const symptoms = data.medical_info.symptoms ?? [];
    ...
    <div className="space-y-8">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          <span className={`px-3 py-1 rounded-lg text-[10px] font-black uppercase tracking-widest shadow-sm ${confidenceColors[confidence] ?? confidenceColors.low}`}>
        ...
        <div className="bg-white border border-slate-200 rounded-2xl p-6 space-y-6 shadow-sm hover:shadow-md transition-shadow">
          ...
          <div className="grid grid-cols-2 gap-y-6 gap-x-4">
            <div className="space-y-1">
              ...
              <p className="text-slate-900 font-semibold text-base">{data.patient_info.patient_name ?? 'N/A'}</p>
            </div>
            <div className="space-y-1">
        ...
        <div className="bg-white border border-slate-200 rounded-2xl p-6 space-y-6 shadow-sm hover:shadow-md transition-shadow">
          ...
          <div className="grid grid-cols-2 gap-y-6 gap-x-4">
            <div className="space-y-1">
              ...
              <p className="text-slate-900 font-semibold text-base">{data.doctor_info.doctor_name ?? 'N/A'}</p>
            </div>
            <div className="space-y-1">
              <p className="text-slate-500 text-[10px] uppercase font-bold tracking-widest">Facility</p>
              <p className="text-slate-900 text-base">{data.doctor_info.clinic_name ?? 'N/A'}</p>
            </div>
            <div className="space-y-1">
            ...
            <div className="space-y-1">
              ...
              <div className="flex gap-2 mt-1">
                {data.doctor_info.doctor_signature_present && (
                  <span className="bg-[#FF4D00]/10 text-[#FF4D00] text-[9px] px-2 py-0.5 rounded-md font-bold border border-[#FF4D00]/20 uppercase">Signature</span>
                )}
                {data.doctor_info.doctor_stamp_present && (
                  <span className="bg-slate-100 text-slate-500 text-[9px] px-2 py-0.5 rounded-md font-bold border border-slate-200 uppercase">Stamp</span>
                )}
                {!data.doctor_info.doctor_signature_present && !data.doctor_info.doctor_stamp_present && (
      ...
      <div className="bg-white border border-slate-200 rounded-2xl overflow-hidden shadow-sm hover:shadow-md transition-shadow">
        <div className="p-6 flex items-center gap-3 text-[#FF4D00] font-bold text-sm uppercase tracking-widest border-b border-slate-100">
          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19.428 15.428a2 2 0 00-1.022-.547l-2.387-.477a6 6 0 00-3.86.517l-.628.285a2 2 0 01-1.96 0l-.628-.285a6 6 0 00-3.86-.517l-2.387.477a2 2 0 00-1.022.547l-1.16 1.16a2 2 0 000 2.828l1.16 1.16a2 2 0 001.022.547l2.387.477a6 6 0 003.86-.517l.628-.285a2 2 0 011.96 0l.628.285a6 6 0 003.86.517l2.387-.477a2 2 0 001.022-.547l1.16-1.16a2 2 0 000-2.828l-1.16-1.16z" />
          </svg>
          Prescribed Regimen
        </div>
              ...
              {medicines.length > 0 ? (
                ...
                <tr>
                  <td colSpan={5} className="px-6 py-12 text-center text-slate-500 font-medium italic">
                    No medications detected in this extraction.
                  </td>
                </tr>
              )}
            </tbody>
          ...
          <div className="space-y-6">
            <div className="space-y-2">
              ...
              <p className="text-slate-900 font-semibold text-base leading-relaxed">
                {data.medical_info.diagnosis_primary ?? 'Not specified'}
              </p>
            </div>
            {symptoms.length > 0 && (
              <div className="space-y-2">
